...
class ScanNetDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=False, load_data=True):
        ...
        with open('/home/dchangyu/mvpnet/scannet_2d3d_feature_val.pkl', 'rb') as f:
            data_fea_label_points_val = pickle.load(f)
        with open('/home/dchangyu/mvpnet/scannet_2d3d_feature_train.pkl', 'rb') as f:
            data_fea_label_points_train = pickle.load(f)
        ...
        if self.set == 'training':
            self.files = data_fea_label_points_train
        elif self.set in ['validation', 'test', 'ERF']:
            self.files = data_fea_label_points_val

        self.cloud_names = [self.files[i]['scan_id'] for i in range(len(self.files))]
    ...
    def load_subsampled_clouds(self):
        ...
        for index in range(len(data)):
            ...
            if exists(KDTree_file):
                ...
                sub_feature_2d3d = data_load['sub_feature_2d3d']
                ...
                points = data[index]['points']  #(n,3) #
                #colors = np.vstack((data['red'], data['green'], data['blue'])).T #(n,3) #grid_sample可以考虑feature(n,d)? #
                #64feature 附加在此？
                feature_2d3d = data[index]['feature_2d3d'] #(n,64)
                # feature_2d3d = np.expand_dims(feature_2d3d,axis=2)
                #labels = data['class'] #(n,) #int32
                labels= data[index]['seg_label'].astype(np.int32) #int64

                # Subsample cloud
                sub_points, sub_feature_2d3d, sub_labels = grid_subsampling(points,
                                                                      features=feature_2d3d,
                                                                      labels=labels,
                                                                      sampleDl=dl)
                ...
                sub_data = {'sub_points': sub_points, 'sub_feature_2d3d' : sub_feature_2d3d, 'sub_labels' : sub_labels}
            ...
            size = sub_feature_2d3d.shape[0] * 4 * 7
        ...
        if self.set in ['validation', 'test']:
            ...
            for index in range(len(data)):
                ...
                if exists(proj_file):
                    ...
                    labels = data[index]['seg_label'].astype(np.int32)
...
class ScanNetCustomBatch:
    ...
    def __init__(self, input_list):
        ...
        # Number of layers
        L = (len(input_list) - 7) // 5 #解释在https://github.com/HuguesTHOMAS/KPConv-PyTorch/issues/69
