...
def select_frames(rgbd_overlap, num_rgbd_frames):
    ...
    for i in range(num_rgbd_frames):
        ...
        # --------------------------------------------
        rgbd_overlap[rgbd_overlap[:, frame_idx]] = False # should choose sphere mask slightly bigger, too small will find no frames
        # --------------------------------------------
...
class ScanNetDataset(PointCloudDataset):
    ...
    def get_rgbd_data(self, sub_rgbd_dict, input_sphere_points, sphere_box, input_sphere_mask, input_sphere_label, whole_scene_points, whole_scene_label):
        ...
        for i, frame_id in enumerate(selected_frames):
            ...
            # image_mask = image_xyz[:, 2] > sphere_box[2] - 1 # (h * w)
            # camera -> world
            ...
            # set invalid flags outsides the sphere box
            # if sphere_box is not None:
            # # sphere_box: (x_min, y_min, z_min, x_max, y_max, z_max)
            #     margin = 0.2
            #     # box_z_min = sphere_box[2] - margin
            #     if margin > 0:
            #         in_chunk_mask = np.logical_and.reduce(
            #             (image_xyz[:, 0] > sphere_box[0] - margin,
            #              image_xyz[:, 0] < sphere_box[3] + margin,
            #              image_xyz[:, 1] > sphere_box[1] - margin,
            #              image_xyz[:, 1] < sphere_box[4] + margin,
            #              # image_xyz[:, 2] > sphere_box[2] - margin,
            #              # image_xyz[:, 2] < sphere_box[5] + margin,
            #              ))
            #         image_mask = np.logical_and(image_mask, in_chunk_mask)
            #     else:
            #         in_chunk_mask = np.logical_and.reduce(
            #             (image_xyz[:, 0] > sphere_box[0] - margin,
            #              image_xyz[:, 0] < sphere_box[2] + margin,
            #              image_xyz[:, 1] > sphere_box[1] - margin,
            #              image_xyz[:, 1] < sphere_box[3] + margin,
            #              # image_xyz[:, 2] > sphere_box[2] - margin,
            #              # image_xyz[:, 2] < sphere_box[5] + margin,
            #              ))
            #
        ...
        # image_mask是为了缩小image_xyz范围,而使查找knn速度更快
        nbrs = NearestNeighbors(n_neighbors=self.k, algorithm='ball_tree').fit(image_xyz_valid) #k (int): k-nn unprojected neighbors of target points
        _, knn_indices = nbrs.kneighbors(input_sphere_points)  # (nc, 3)
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            # Indices of sphere mask for input region, should bigger then input sphere
            ...
            sphere_box_min = np.min(input_points_feat_aggre[:,:3], axis=0) #(x_min, y_min, z_min)
            sphere_box_max = np.max(input_points_feat_aggre[:,:3], axis=0) #(x_max, y_max, z_min)

            #input sphere mask should be bigger than input sphere
            # determine region around center
            chunk_min = center_point[0][:2] - self.config.in_radius
            chunk_max = center_point[0][:2] + self.config.in_radius
            # Magic numbers are referred to the original implementation.
            # select points within the block with a margin
            xy = points[:, :2]
            chunk_mask = np.all(np.logical_and(xy >= (chunk_min - 0.1),
                                               xy <= (chunk_max + 0.1)),
                                                axis=1)

            # sphere_mask = np.all(np.logical_and(xy >= (sphere_box_min - 0.2),
            #                                    xy <= (sphere_box_max + 0.2)),
            #                                     axis=1)

            input_points_inds = np.zeros(points.shape[0], dtype=bool) #需要wohle scence, 已经subpoints 在whole scence上的index
            input_points_inds[input_sphere_mask_inds] = True
            # input_sphere_mask = chunk_mask
            # input_sphere_mask = sphere_mask
            input_sphere_mask = input_points_inds

            # ---------------------------------------------------------------------------- #
            # Resample points to a fixed number
            # ---------------------------------------------------------------------------- #
            # chunk_nb_pts = len(input_points_feat_aggre)
            # pts = 7192
            # # pts = 6192
            # # assert chunk_nb_pts > pts, 'Mismatch points number: {} < {}.'.format(chunk_nb_pts, pts)
            # if chunk_nb_pts < pts:
            #     pad = np.random.randint(chunk_nb_pts, size=pts - chunk_nb_pts)
            #     crop_pad_choice = np.hstack([np.arange(chunk_nb_pts), pad])
            #     # update project indices and validation labels when add extra overlap points?
            #     # if self.set == 'validation':
            #     #     see_proj_inds = self.test_proj[cloud_ind][input_inds]
            #     #     more_overlap_proj_inds = see_proj_inds[crop_pad_choice]
            #         # self.validation_labels[cloud_ind][input_inds] = self.validation_labels[cloud_ind][input_inds][crop_pad_choice]
            # else:
            #     crop_pad_choice = np.random.choice(chunk_nb_pts, size=pts, replace=False)
            #
            #
            # input_chunk_points = input_points[crop_pad_choice]
            # input_chunk_inds = input_inds[crop_pad_choice]
            # #-----------------------------------------------------
            # input_feat_chunk_points = input_points_feat_aggre[crop_pad_choice]
            # input_chunk_seg_label = input_labels[crop_pad_choice]

            # ---------------------------------------------------------------------------- #
            # On-the-fly find frames to maximally cover the sphere
            # ---------------------------------------------------------------------------- #


            if self.num_rgbd_frames > 0:
                # bounding box: (x_min, y_min,z_min, x_max, y_max, z_max)
                sphere_box = np.hstack([sphere_box_min - (0.3,0.3,0.3), sphere_box_max + (0.3,0.3,0.3)])
                # sphere_box = np.hstack([sphere_box_min - 0.5, sphere_box_max + 0.5])
                # sphere_box = np.hstack([chunk_min - 0.5, chunk_max + 0.5])
                ...
                # images, image_xyz, knn_indices, image_mask = self.get_rgbd_data(self.sub_rgbd_dict[cloud_ind],
                #                                                                 input_feat_chunk_points,
                #                                                                 sphere_box,
                #                                                                 input_sphere_mask,
                #                                                                 input_chunk_seg_label,
                #                                                                 points,
                #                                                                 whole_scene_label,
                #                                                                 )
            ...
            # image_xyz = image_xyz[0] # (nv, 3, h, w)
            # knn_indices = knn_indices[0] # original output is tuple
            ...
            input_3d_features = input_colors

            t += [time.time()]

            # Stack batch
            p_list += [input_points]
            color_list += [input_3d_features]
            #--------------------------------
            p_f_list += [input_points_feat_aggre]
            # p_f_list += [input_feat_chunk_points]
            image_xyz_list += [image_xyz]
            images_list += [images]
            knn_list += [knn_indices]
            #---------------------------
            l_list += [input_labels]
            # l_list += [input_chunk_seg_label]
            pi_list += [input_inds]
            # pi_list += [input_chunk_inds]
            i_list += [point_ind]
            ci_list += [cloud_ind]
            s_list += [scale]
            R_list += [R]
        ...
        color_features = np.concatenate(color_list, axis=0) #(np,3)
        ...
        input_list += [scales, rots, cloud_inds, point_inds, input_inds, knn_list, color_features]
...
class ScanNetCustomBatch:
    ...
    def __init__(self, input_list):
        ...
        ind += 1
        self.knn_indices = torch.from_numpy(input_list[ind])
        #--------------------------------------------------------------------------
        ind += 1
        ...
        ind += 1
        self.colors = torch.from_numpy(input_list[ind])
    ...
    def pin_memory(self):
        ...
        self.knn_indices = self.knn_indices.pin_memory()
    ...
    def to(self, device):
        ...
        self.knn_indices = self.knn_indices.to(device)
        #=============================
