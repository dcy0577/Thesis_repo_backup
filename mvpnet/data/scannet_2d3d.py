...
class ScanNet2D3DChunks(Dataset):
    ...
    def _load_dataset(self):
        with open(osp.join('/home/dchangyu/mvpnet/',self.cache_dir, 'scannetv2_{}.pkl'.format(self.split)), 'rb') as f:
    ...
    def __getitem__(self, index):
        ...
        if not flag:
            chunk_min = np.min(points[:, :2], axis=0) #axis = 0 返回 (xmin,ymin)每列最小值
...
def test_ScanNet2D3DChunksTest():
    from mvpnet.utils.o3d_util import visualize_point_cloud
    from mvpnet.utils.visualize import visualize_labels
    from mvpnet.utils.visualize import pc_with_labels
    cache_dir = osp.join('/home/dchangyu/mvpnet/ScanNet/cache_rgbd')
    image_dir = osp.join('/home/dchangyu/mvpnet/ScanNet/scans_resize_160x120')
    ...
    for i in range(len(dataset)):
        ...
        for data in data_list:
            ...
            #name = 'pc'+str(i)+'.ply'
            #o3d.io.write_point_cloud('/home/dchangyu/mvpnet/'+name, pc_with_labels(points, seg_label, style='scannet'))
...
#-----------------------------------------------------------------------------------------------------
# For each scene, return its whole scene point cloud and frames that cover whole scene, not chunk!
#------------------------------------------------------------------------------------------------------
class ScanNet2D3DWhole(Dataset):
    """ScanNetV2 2D-3D Whole Scene dataset"""
    label_id_tsv_path = osp.join(_META_DIR, 'scannetv2-labels.combined.tsv')
    scannet_classes_path = osp.join(_META_DIR, 'labelids.txt')
    split_dir = _META_DIR
    split_map = {
    ...
    def __init__(self,
        ...
        Args:
            ...
            to_tensor (bool): whether to convert to torch.Tensor
        """
        super(ScanNet2D3DWhole, self).__init__()
        ...
        self.nyu40_to_scannet = np.full(shape=41, fill_value=self.ignore_value, dtype=np.int64)
        self.nyu40_to_scannet[list(self.scannet_mapping.keys())] = np.arange(len(self.scannet_mapping))
        # self.nyu40_to_scannet[list(self.scannet_mapping.keys())] = np.array(list(self.scannet_mapping.keys()))
    ...
    #delete chunk_box in below
    def get_rgbd_data(self, data_dict, chunk_points, chunk_mask):  #chunk_points=points, chunk_mask=whole scene seg label
        scan_id = data_dict['scan_id']
        scan_dir = osp.join(self.image_dir, scan_id)
        paths = self._get_paths(scan_dir)
    ...
    def __getitem__(self, index):
        ...
        # ---------------------------------------------------------------------------- #
        # Note that astype will copy data.
        points = data_dict['points'].astype(np.float32) #(n,3)
        seg_label = data_dict['seg_label']
        # label:[0,1,2,...,19] -100 ignored
        seg_label = self.nyu40_to_scannet[seg_label].astype(np.int64)
        ...
        # Resample points to a fixed number : 8192
        ...
        #     crop_pad_choice = np.hstack([np.arange(chunk_nb_pts), pad])
        # else:
        #     crop_pad_choice = np.random.choice(chunk_nb_pts, size=self.nb_pts, replace=False) #nb_pts defined in config: mvpnet_3d: 8192
        # chunk_points = chunk_points[crop_pad_choice]
        # chunk_seg_label = chunk_seg_label[crop_pad_choice]
        ...
        # ---------------------------------------------------------------------------- #
        # On-the-fly find frames to maximally cover the chunk (chunk=whole scene)
        # ---------------------------------------------------------------------------- #
        if self.num_rgbd_frames > 0:
            # bounding box: (x_min, y_min, x_max, y_max)
            #chunk_box = np.hstack([chunk_min - self.chunk_margin, chunk_max + self.chunk_margin])
            out_dict_rgbd = self.get_rgbd_data(data_dict, chunk_points, chunk_mask) ##chunk_points=points, chunk_mask=whole scene seg label
            out_dict.update(out_dict_rgbd)
    ...
    def __str__(self):
        base_str = '{:s}: {} classes with {} scenes and {} frames.'.format(
            self.__class__.__name__, len(self.class_names), len(self.scan_ids), self.total_frames)
        # attr_list = ['nb_pts', 'num_rgbd_frames', 'resize',
        #              'image_normalizer', 'k', 'z_rot', 'flip']
        attr_list = ['num_rgbd_frames', 'resize',
                     'image_normalizer', 'k', 'z_rot', 'flip']
        extra_str = ', '.join([x + '=' + str(getattr(self, x)) for x in attr_list])
        return base_str + '\n' + extra_str


def test_ScanNet2D3DWhole():
    from mvpnet.utils.o3d_util import visualize_point_cloud
    from mvpnet.utils.visualize import visualize_labels
    from mvpnet.utils.visualize import pc_with_labels
    from mvpnet.utils.o3d_util import draw_point_cloud
    cache_dir = osp.join('/home/dchangyu/mvpnet/ScanNet/cache_rgbd_3scene')
    image_dir = osp.join('/home/dchangyu/mvpnet/ScanNet/scans_resize_160x120')

    #np.random.seed(0)
    dataset = ScanNet2D3DWhole(cache_dir=cache_dir,
                                image_dir=image_dir,
                                split='val',
                                #nb_pts=8192,
                                num_rgbd_frames=30,
    ...
    for i in range(1):
        ...
        image_mask = data['image_mask']
        image_xyz_valid = image_xyz[image_mask]
        print('image_xyz_valid',image_xyz_valid.shape, image_xyz_valid.dtype)
        knn_indices = data['knn_indices']


        pcd_xyz = draw_point_cloud(image_xyz[image_mask], [0., 1., 0.])
        pcd_xyz = draw_point_cloud(image_xyz.reshape(-1, 3)[knn_indices[:, 2]], [0., 1., 0.])

        pcd_scene = draw_point_cloud(points, [1., 0., 0.])
        # store pc data
        print('store pc data' + str(i))
        # name = 'pc_original' + str(i) + '.ply'
        # o3d.io.write_point_cloud('/home/dchangyu/mvpnet/' + name, pcd_scene)
        # name2 = 'pc_image' + str(i) + '.ply'
        # o3d.io.write_point_cloud('/home/dchangyu/mvpnet/' + name2, pcd_xyz)



        #visualize_point_cloud(image_xyz_valid)
        name = 'pc_image_color'+str(i)+'.ply'
        o3d.io.write_point_cloud('/home/dchangyu/mvpnet/'+name, pc_with_labels(image_xyz.reshape(-1, 3)[knn_indices[:, 2]], seg_label))

        # knn_indices = data['knn_indices']
        # print('image_xyz_reshape', image_xyz.reshape(-1, 3)[knn_indices[:, 2]].shape)
        # print('images_reshape', images.reshape(-1, 3)[knn_indices[:, 2]].shape)

        #name = 'pc'+str(i)+'.ply'
        #o3d.io.write_point_cloud('/home/dchangyu/mvpnet/'+name, draw_point_cloud(image_xyz_valid.reshape(-1, 3)[knn_indices[:, 2]], [0., 1., 0.]))
        ...
        #     image_xyz = image_xyz.permute(0,4, 1, 2, 3).reshape(1, 3, 20 * 120 * 160)
        #
        #     image_xyz = group_points(image_xyz.cuda(), knn_indices.cuda())  # (b, 3, np, k)
        #
        # print('group_points',image_xyz.shape)

if __name__ == '__main__':
    test_ScanNet2D3DWhole()
