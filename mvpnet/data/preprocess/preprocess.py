...
# SCAN_DIR = 'scans'
DATA_DIR = '/home/dchangyu/mvpnet/ScanNet'
SCAN_DIR = 'scans_resize_160x120'
#这里把testdata也存到scans里面了，原本应该存在scans_test里面
SCAN_TEST_DIR = 'scans'
META_DIR = '/home/dchangyu/mvpnet/mvpnet/data/meta_files'
...
def process_scan_3d_sem_seg(scan_id, is_test=False, compute_rgbd_overlap=False, output_dir=None):
    ...
    if compute_rgbd_overlap:
        # get frames in scan
        glob_path = osp.join(scan_dir, 'color', '*') #can be .jpg can be .png
...
def main():
    ...
    func = partial(process_scan_3d_sem_seg, compute_rgbd_overlap=args.rgbd, is_test=is_test)#简单总结functools.partial偏函数的作用就是，把一个函数的某些参数给固定住（也就是设置默认值），返回一个新的函数，调用这个新函数会更简单。
...
def test():
    ...
    process_scan_3d_sem_seg(scan_id, compute_rgbd_overlap=True, is_test=False)
