...
def compute_3d_weights():
    pkl_path = osp.expanduser('/home/dchangyu/mvpnet/ScanNet/cache_rgbd/scannetv2_train.pkl')
    ...
    for data_dict in pickle_data:
        ...
        label_weights += np.bincount(label, minlength=41) #给出每个label被索引的次数

    label_weights = label_weights.astype(np.float32)[VALID_CLASS_IDS]
    label_weights = label_weights / np.sum(label_weights)
    label_log_weights = 1 / np.log(1.2 + label_weights)

    np.savetxt('scannetv2_train_3d_log_weights_20_classes_new.txt', label_log_weights)


def compute_2d_weights():
    dataset = ScanNet2D('ScanNet', 'train', resize=(160, 120))
    ...
    np.savetxt('scannetv2_train_2d_log_weights_20_classes_new.txt', label_log_weights)
